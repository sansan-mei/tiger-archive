...
(function (root, factory) {
  const api = factory(
    typeof module === "object" && module.exports
      ? require("../core/abilities.js")
      : root.TankAbilities,
  );
  if (typeof module === "object" && module.exports) module.exports = api;
  else root.TankPlugins = api;
})(typeof window === "undefined" ? globalThis : window, function (Abilities) {
  "use strict";
  ...
  function createRegistry() {
    const tanks = Object.create(null),
    ...
    function register(plugin) {
      ...
      if (
        !plugin ||
        !["tank", "weapon"].includes(plugin.kind) ||
        !/^[a-z][a-z0-9-]{0,31}$/.test(plugin.id) ||
        !/^\d+\.\d+\.\d+$/.test(plugin.version) ||
        plugin.apiVersion !== 1 ||
        typeof plugin.buildVisual !== "function"
      )
        throw new Error("Invalid plugin metadata");
      const table = plugin.kind === "tank" ? tanks : weapons;
      if (Object.hasOwn(table, plugin.id))
      ...
      const s =
        plugin.kind === "tank"
          ? {
              ...plugin.spec,
              abilityCooldown:
                Abilities.definitions[plugin.spec?.ability]?.cooldown,
              abilityDuration:
                Abilities.definitions[plugin.spec?.ability]?.duration,
            }
          : plugin.spec;
      if (!s || typeof s.name !== "string" || !s.name.length)
      ...
      const ranges =
        plugin.kind === "tank"
          ? {
              hp: [1, 1000],
              shield: [0, 1000],
              abilityCooldown: [1, 3600],
              abilityDuration: [1, 600],
              speed: [1, 30],
              reverse: [1, 15],
              accel: [1, 50],
              turn: [0.1, 5],
      ...
      if (plugin.kind === "tank") {
        if (
          !Object.hasOwn(Abilities.definitions, s.ability) ||
          !Number.isInteger(s.shield) ||
          s.abilityDuration > s.abilityCooldown
        )
          throw new Error("Invalid ability");
        if (
          (s.movement !== undefined && s.movement !== "strafe") ||
          (s.muzzleScale !== undefined &&
            (!Number.isFinite(s.muzzleScale) ||
              s.muzzleScale < 0.1 ||
              s.muzzleScale > 1)) ||
          (s.height !== undefined &&
            (!Number.isFinite(s.height) || s.height < 1 || s.height > 3))
        )
          throw new Error("Invalid infantry dimensions");
        if (
          !Array.isArray(s.mount) ||
          s.mount.length !== 3 ||
          !s.mount.every((n) => Number.isFinite(n) && Math.abs(n) <= 4)
        )
        ...
        if (s.splashDamage !== undefined || s.splashRadius !== undefined)
          if (
            ...
            s.splashDamage < 1 ||
            s.splashDamage > 200 ||
            !Number.isFinite(s.splashRadius) ||
            s.splashRadius < 1 ||
            s.splashRadius > 12 ||
      ...
      table[plugin.id] = freeze({
        ...plugin,
        spec: JSON.parse(JSON.stringify(s)),
      });
      return table[plugin.id];
    }
    function seal() {
      if (!Object.keys(tanks).length || !Object.keys(weapons).length)
        throw new Error("Empty plugin catalogue");
      if (!locked) {
        manifest = canonical(
          [...Object.values(tanks), ...Object.values(weapons)]
            .map((p) => ({
              kind: p.kind,
              id: p.id,
              version: p.version,
