...
- 自由混战正常比赛为 8 分钟模拟时间，或先到 15 次击毁结束；同分时比较死亡数，两项相同为平局。自由混战房间另有 15 分钟墙钟时间的异常兜底，不能当作正常赛制。僵尸合作生存按自身 16 分钟时限结束，房间墙钟兜底也使用相同的 PvE 时限，不会在第 15 分钟提前判负。大厅 30 分钟过期，结算后 5 分钟清理；房主可在结算后返回大厅再次准备。
