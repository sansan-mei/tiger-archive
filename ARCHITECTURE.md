...
当前采用分模块的 JavaScript 单体：浏览器负责操作和表现，Node 负责联机权威模拟。保留 CommonJS / 浏览器双入口，不引入打包器、ECS 框架或额外服务。协议 v43；开发规则以共享核心为准。双管霰弹由权威核心维护两发弹匣、换枪暂存 `pve.magazines` 及 `scatterMarks` / `scatterSlows`；整发弹片结算后才发布第二发易伤，客户端只读取弹量、冷却与有效武器配置。
