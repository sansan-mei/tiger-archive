...
当前采用分模块的 JavaScript 单体：浏览器负责操作和表现，Node 负责联机权威模拟。保留 CommonJS / 浏览器双入口，不引入打包器、ECS 框架或额外服务。协议 v17；开发规则以共享核心为准。
...
PvE 不走 FFA 的自动复活或击杀上限结算。核心模拟 60 Hz，PvE 广播 10 Hz、FFA 20 Hz。AI 寻路按敌人槽位分摊到不同 tick，模型仅在该玩家换装时重建，不重建整场角色。首版重用三层地图；未来增大尸群，应把敌人状态投影改成紧凑格式或独立兴趣区域同步，再评估更大实体池。
