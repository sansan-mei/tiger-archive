...
当前采用分模块的 JavaScript 单体：浏览器负责操作和表现，Node 负责联机权威模拟。保留 CommonJS / 浏览器双入口，不引入打包器、ECS 框架或额外服务。协议 v29；开发规则以共享核心为准。
...
| core/modules.js | 十种通用模块、状态计时、受遮挡的有界次生效果与快照校验；供所有武器复用 |
