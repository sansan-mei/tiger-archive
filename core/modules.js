...
(function(root,factory){
  ...
  const rewards=Object.freeze({
    modEmber:{name:"燃烧弹芯",description:"直接命中点燃敌人4秒，每秒20伤害",module:true},
    modFrost:{name:"冰霜弹芯",description:"直接命中令敌人减速45%，持续4秒",module:true},
    modFracture:{name:"破甲刻印",description:"直接命中令敌人进入破甲状态4秒",module:true},
    modCombustion:{name:"燃爆装置",description:"命中燃烧目标引爆5米范围，对最多4名其他敌人各造成80伤害",module:true},
  ...
  function onHit(b,hit,shot,before) {
    ...
    if(target.alive&&u.modFrost)target.slowUntil=Math.max(target.slowUntil,b.tick+240);
    if(u.modOverload) {
  ...
  function onDeath(b,target,owner) {
    ...
    if(!b.moduleSecondary&&u?.modReprisal&&state?.fractureUntil>b.tick)burst(b,owner,from,5,60,target.id);
  ...
  function tick(b) {
    for(const [id,state] of Object.entries(b.pve.moduleStatus)) {
      ...
      if(state.burnUntil>=b.tick && state.burnNext>0 && b.tick>=state.burnNext) {
        ...
        b.damage(target,20,state.burnOwner,{x:target.x,y:target.y+1.5,z:target.z});
      }
