...
test('death respawns after four seconds, preserves score and grants cancellable protection',()=>{
 const b=battle(),[a,v]=b.entities;b.damage(v,10000,a.id,v);assert.equal(v.deaths,1);assert.equal(a.kills,1);
...
test('occupied spawn points postpone respawn, forfeited players never return',()=>{
 const b=battle(),v=b.entities[1];b.damage(v,10000,'p1',v);b.tick=v.respawnAt-1;const valid=b.valid.bind(b);b.valid=()=>false;b.step();assert.equal(v.alive,false);
...
test('fifteenth kill ends the match once; timer tie is a draw and lower deaths break ties',()=>{
 const b=battle();b.entities[0].kills=14;b.damage(b.entities[1],10000,'p1',b.entities[1]);assert.equal(b.step().filter(e=>e.type==='end').length,1);assert.equal(b.winnerId,'p1');assert.deepEqual(b.step(),[]);
...
test('repair and boost are authoritative, respect height, refresh after 20 seconds and survive restore',()=>{
 ...
 Object.assign(a,{floor:0,y:0});b.step();assert.equal(a.hp,70);assert.equal(repair.readyAt,b.tick+1200);b.step();assert.equal(a.hp,70);
 ...
 b.tick=repair.readyAt;Object.assign(a,{x:repair.x,z:repair.z,hp:30});b.step();assert.equal(a.hp,70);
});
