...
test("dash moves about thirteen metres, can fire and respects a wall and ramp sides", () => {
  const b = make("light"),
    a = b.entities[0];
  const z = a.z;
  const events = b.step({ a: { ability: true, fire: true } });
  assert.ok(events.some((e) => e.type === "shot"));
  for (let i = 0; i < 29; i++) b.step({ a: { ability: true } });
  assert.ok(Math.abs(z - a.z - 13) < 0.01);
  b.map.obstacles = [{ id: "block", floor: 0, x: 0, z: 35, w: 12, d: 2, h: 4 }];
  for (let i = 0; i < 120; i++) b.step({ a: { forward: true } });
  assert.ok(a.z > 38);
  Object.assign(a, {
    x: -26,
    z: 16,
  ...
  for (let i = 0; i < 30; i++) b.step({ a: { ability: true } });
  assert.ok(a.x > -30, "cannot dash through ramp sides");
});
test("held skill does not retrigger, barrier absorbs damage then expires and cooldown persists", () => {
  ...
  assert.equal(events.filter((e) => e.type === "ability").length, 0);
  assert.equal(a.barrier, 0);
  b.step();
  assert.ok(b.step({ a: { ability: true } }).some((e) => e.type === "ability"));
  assert.equal(a.barrier, 60);
});
...
test("ability and shield state survives authority/replica and checkpoint continuation", () => {
  for (const tankType of Object.keys(C.TANKS)) {
    ...
    session.start();
    const events = session.advance(1 / 60, { ability: true });
    assert.ok(events.some((e) => e.type === "ability"));
    S.validateSnapshot(session.current());
    const b = session.authority.battle,
      copy = new C.Battle();
    copy.restore(b.snapshot());
    for (let i = 0; i < 90; i++) assert.deepEqual(b.step(), copy.step());
    assert.deepEqual(b.snapshot(), copy.snapshot());
    const bad = b.snapshot();
    bad.entities[0].shield = 99999;
...
test("continuous full-power weapon hits meet the 2/3/4 laser baseline and balanced cannon counts", () => {
  ...
  for (const [weaponType, counts] of Object.entries(expected))
    for (const [i, tankType] of ["light", "medium", "heavy"].entries()) {
      ...
      b.start();
      const [a, target] = b.entities;
      Object.assign(a, { x: 0, z: 30 });
        ...
        for (const event of events)
          if (event.type === "damage" && event.id === "b") {
            ...
            assert.equal(
              target.alive,
              hits < counts[i],
              weaponType + " / " + tankType + " at hit " + hits,
            );
          }
      }
      assert.equal(hits, counts[i], weaponType + " / " + tankType);
