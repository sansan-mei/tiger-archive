...
test("page event wiring creates a room, starts, renders snapshots, pauses locally and leaves", async () => {
  ...
  const fixedRotation=renderedCamera.quaternion.clone();
  canvas.emit('pointermove',{clientX:960,clientY:200});
  advance(1);
  assert.equal(nodes.get('aim-reticle').style.left,'75%');
  assert.equal(nodes.get('aim-reticle').style.top,'25%');
  assert.ok(renderedCamera.quaternion.angleTo(fixedRotation)<1e-7,'PvE aim does not rotate camera');
  // Fire edges must carry horizontal aim even without a render refresh.
  canvas.emit('pointerdown',{pointerType:'mouse',button:0,clientX:1000,clientY:650});
  assert.equal(coop.authority.commands.get(survival.entities[0].id).aimPitch,0);
  assert.equal(coop.authority.commands.get(survival.entities[0].id).fire,true);
  canvas.emit('pointerup');
  canvas.emit('pointermove',{clientX:240,clientY:150});
  ...
  const beforeMove={x:survival.entities[0].x,z:survival.entities[0].z};
  for(const fn of windowEvents.keydown)fn({code:'KeyW',preventDefault(){},repeat:false});
  advance(6);
