...
const P=require('../plugins/catalog.js'),C=require('../battle-core.js'),S=require('../battle-session.js');
const root=path.resolve(__dirname,'..');
function browser(beforeCore){
  const context=vm.createContext({TextEncoder});context.window=context;
  const html=fs.readFileSync(path.join(root,'index.html'),'utf8');
  for(const [,src] of html.matchAll(/<script src="([^"]+)"/g)){
    if(src.startsWith('https:')||src==='vendor/three.min.js'||src==='battle.js')continue;
    if(src==='battle-core.js'&&beforeCore)beforeCore(context);
...
test('browser script order registers the same plugins and protocol as Node',()=>{
  const b=browser();assert.equal(b.TankBattle.PLUGIN_MANIFEST,C.PLUGIN_MANIFEST);
...
test('registration rejects duplicates, invalid stats and incompatible plugin APIs',()=>{
  const r=P.createRegistry(),tank=P.tanks.light,weapon=P.weapons.rapid;
  r.register(tank);assert.throws(()=>r.register(tank),/Duplicate/);
  for(const changes of [{apiVersion:2},{spec:{...weapon.spec,cooldown:0}},{spec:{...weapon.spec,delivery:'unknown'}},{spec:{...weapon.spec,damage:NaN}}])assert.throws(()=>r.register({...weapon,...changes}));
  r.register(weapon);r.seal();assert.throws(()=>r.register({...tank,id:'new'}),/locked/);
...
test('manifest ignores registration order but detects changed stats or versions',()=>{
  function manifest(items){const r=P.createRegistry();items.forEach(p=>r.register(p));return r.seal();}
  const t=P.tanks.medium,w=P.weapons.standard;
  assert.equal(manifest([t,w]),manifest([w,t]));
  assert.notEqual(manifest([t,w]),manifest([t,{...w,version:'1.0.1'}]));
  assert.notEqual(manifest([t,w]),manifest([t,{...w,spec:{...w.spec,damage:37}}]));
});
test('an additional weapon works without an ID branch in the core',()=>{
  const ctx=browser(b=>vm.runInContext("TankPlugins.register({...TankPlugins.weapons.standard,id:'pulse',spec:{...TankPlugins.weapons.standard.spec,delivery:'ray',damage:120,range:90,cooldown:20}})",b));
  vm.runInContext(`
...
test('plugin mismatch is rejected at welcome, snapshot and restore boundaries',()=>{
  const a=new S.Authority(),welcome=a.attach('peer','p1'),r=new S.Replica();
  assert.throws(()=>r.welcome({...welcome,pluginManifest:'different'}));r.welcome(welcome);
...
test('cancelFire takes priority over simultaneous fire for every weapon',()=>{
  for(const weaponType of Object.keys(C.WEAPONS)){
    const b=new C.Battle({participants:C.defaultParticipants({tankType:'medium',weaponType}).map(p=>({...p,controller:'human'}))});b.start();
    assert.equal(b.step({p1:{cancelFire:true,fire:true}}).filter(e=>e.type==='shot').length,0);
    assert.equal(b.getEntity('p1').charge,0);
  }
});
test('packet size limit counts UTF-8 bytes, including object payloads',()=>{
  const raw={text:'虎'.repeat(24000)};assert.throws(()=>S.decode(raw),/too large/);assert.throws(()=>S.decode(JSON.stringify(raw)),/too large/);
});
