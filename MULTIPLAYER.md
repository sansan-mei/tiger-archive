...
当前协议 v12 包含 ability 布尔输入与 ability 事件。护盾、临时屏障、最近交火 tick、技能有效期、冷却和按键边沿状态全部由权威核心维护并进入快照，检查点恢复时保留。车体 light/medium/heavy 2.1.1、human 1.0.1；武器 standard/rapid 2.1.2、laser 2.2.2、rocket 1.0.2，握手拒绝旧清单。维修包随装甲数值调整为 40。
...
先做两台设备的真实 WebSocket 联调，再验证 8 人、弱网、代理、Redis 和重启恢复。121 项自动化测试是模拟验证，不能证明这些部署场景已通过。
...
红色瞄准标记由客户端可见场景射线产生，不作为命中请求发送。真实命中仍由服务器根据瞄准输入、炮口和 ray 武器 beamRadius 计算，伤害/射程不变。beam 事件新增 radius（0–1 米，未声明宽度的旧式 ray 为 0），由副本校验并用于显示。激光插件 2.2.2；协议和 Redis 前缀升级 v12。
