...
协议当前为 v44。PvE 霰弹分支使用运行时有效武器覆盖（2 发弹匣、18 tick 间隔、120 tick 换弹、关闭基础暴击），不修改 standard 插件或基础／重炮流；第二发整发结算后施加 15% 易伤 240 tick，不另发主炮。握手及完整检查点携带 pluginManifest，包含稳定序列化的插件 ID、版本、API 版本、配置，以及共用技能和赛制规则。握手与恢复拒绝不一致清单。网络广播不重复清单和 AI brain；Replica 在已核对的握手后验证广播字段。
