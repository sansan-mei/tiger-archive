...
协议当前为 v43。PvE 霰弹分支使用运行时有效武器覆盖（2 发弹匣、18 tick 间隔、120 tick 换弹、关闭基础暴击），不修改 standard 插件或基础／重炮流；第二发整发结算后施加 15% 易伤 240 tick，不另发主炮。握手及完整检查点携带 pluginManifest，包含稳定序列化的插件 ID、版本、API 版本、配置，以及共用技能和赛制规则。握手与恢复拒绝不一致清单。网络广播不重复清单和 AI brain；Replica 在已核对的握手后验证广播字段。
...
ray 武器基础配置可声明 beamRadius，范围 0.05–1 米；省略则保持原细射线判定。PvE 权威升级允许运行时有效半径提高到 1.5 米，网络事件上限为 2 米。只用于 ray，不适用于 projectile。激光插件当前为 2.5.0，基础 beamRadius: 0.45 和 penetratesBodies: true。核心对实体和遮挡物一起扩张判定，beam 事件携带实际 radius，显示层按同一半径绘制。当前协议 v43；车体 light/medium/heavy 2.2.0、human 1.0.1；武器 standard 2.2.1、rapid 2.1.4、laser 2.5.0、rocket 1.0.4。
...
当前 zombie 1.5.0 / pistol 1.1.0，均为原创程序化外观。zombie 声明 enemyOnly: true，只能由 PvE 创建为 bot 实体；PLAYER_TANKS 排除它，TANKS 保留碰撞参数。感染者复用 standard 武器的冷却字段，但 AI 只做近战，不调用开火或 Shift 技能，模型不绘制枪械。
...
僵尸通过 spec.variants 共享五种配置，外观钩子接收 zombieType；无需注册五个玩家车体。variants 参数进入插件清单，改动时应提升 zombie 插件版本。人数血量缩放系数来自 RULES.pveHealthPerPlayer；v34 对非 Boss 怪额外乘 0.5 仅在 PvE.healthFor 生效，换装和人数不会修改冻结的插件基础数值。
