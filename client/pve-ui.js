...
(window.TankClient ??= {}).createPveUI = function ({ C, $, choose }) {
  ...
  return {
    update(state, id) {
      ...
      if (pve) {
        const boss = state.entities.find(e => e.zombieType === "boss");
        ...
        $("pve-boss").textContent = boss?.alive ? "零号感染体 " + Math.ceil(boss.hp) + "/" + boss.maxHp + (pve.boss.telegraph ? " · 红圈即将爆发，离开落点！" : boss.hp < boss.maxHp/2 ? " · 狂暴" : "") : "7:00 最终 Boss 登场 · 8:00 前击败";
      }
      ...
      $("mission-rule").textContent = pve
        ...
        : "8 分钟 · 先到 15 次击毁";
      $("score-title").textContent = pve ? "合作肉鸽 · 击败最终 Boss" : "计分板 · 15 次击毁获胜";
